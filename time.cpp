...
TimeInterval operator+(TimeInterval & t, TimeInterval & ti) {
	int m = 0;
	string result[SIZE];
	//int  M = stoi(t.m) + stoi(ti.m), H = stoi(t.h) + stoi(ti.h), MM = stoi(t.mm) + stoi(ti.mm), DD = stoi(t.dd) + stoi(ti.dd), YY = stoi(t.yy) + stoi(ti.yy);
	int time[SIZE]= {stoi(t.m) + stoi(ti.m), stoi(t.h) + stoi(ti.h), stoi(t.dd) + stoi(ti.dd), stoi(t.mm) + stoi(ti.mm), stoi(t.yy) + stoi(ti.yy)};
	...
	if (time[4] == 0)
		...
		result[4] = to_string(time[4]);
	TimeInterval t1(result[1], result[0], result[2], result[3], result[4]);
	return t1;
}
...
TimeInterval operator-(Time & t, Time & ti) {
	string result[SIZE];
	int time[SIZE]= {stoi(t.m) - stoi(ti.m), stoi(t.h) - stoi(ti.h), stoi(t.dd) - stoi(ti.dd), stoi(t.mm) - stoi(ti.mm), stoi(t.yy) - stoi(ti.yy)};
	for (int i = 0; i < SIZE - 1; i++) {
		if (time[i] < 0) {
...
int main() {
	Time t1("13", "45", "01", "06", "2019"), t2;
	TimeInterval ti1("01", "07", "21", "01", "0001"), ti2("13", "45", "01", "06", "2019"), ti3;
	cout << "time1: ";
	//t1.show();
	ti2.show();
	cout << "time2: ";
